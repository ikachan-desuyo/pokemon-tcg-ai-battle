...
def _side(p):
    ...
    return {
        "prizes": prizes_left,
        "hp_ratio": (act.get("hp", 0) / act["maxHp"]) if (act and act.get("maxHp")) else 0.0,
        "active_energy": _en(act) if act else 0,
        "bench": len(bench),
        "board_energy": (_en(act) if act else 0) + sum(_en(s) for s in bench),
...
def encode_state(cur, who):
    """tools/value_model.py の FEATURES と同順・同定義のベクトル(dict/asdict両対応)。"""
    me = _side(cur["players"][who]); opp = _side(cur["players"][1 - who])
    return [
        me["prizes"], opp["prizes"], opp["prizes"] - me["prizes"],
        me["hp_ratio"], opp["hp_ratio"], me["active_energy"], opp["active_energy"],
