...
HERO_CAPE = 1159     # ヒーローマント(どうぐ): つけたポケモンの最大HP+100。耐久の核
...
class ArchaludonBot(DeckBot):
    ...
    def __init__(self, plan=None, decklist=None):
        ...
        # matchup_plans: ベースと違う『相手別差分knob』のみ(DRY, 残りはベースPLAN参照)。ブリジュラス固有の戦術。
        # 実#1ログの対面別分析: 1位はデッキ非依存でなく相手で動きを変える(vs Mega系=積極Metal Defender・初攻撃T3.5、
        #   vs Dragapult=慎重・初攻撃T4.5・攻撃%低)。ただし『攻撃選択』の対面差はベースの最大ダメージKOが既に吸収する
        #   (A/B: 攻撃prefのmatchup_planは中立)ため入れない。Dragapultの慎重展開等、検証で有効な差分のみ今後追加する。
        self.matchup_plans = {}
    ...
    def _pick_cape(self, idxs, options, hand, me):
        """ヒーローマント(+100HP)を攻撃役に装着＝耐久の核。相手の火力(Nebula 210等)をFull Metal Lab(-30)と
        併せて耐えるため、まだどうぐの無い攻撃役に最優先で貼る。前(active)の攻撃役>ベンチ、ex>ジュラルドンの順。"""
        best, best_key = None, None
        for i in idxs:
            op = options[i]
            if self._hand_id(hand, op.index) != HERO_CAPE:
                continue
            spot = self._target_spot(me, op.in_play_area, op.in_play_index)
            if not spot or spot.get("id") not in (ARCH_EX, DURALUDON):
                continue                              # 攻撃役にのみ装着(壁/エンジンには貼らない)
            if spot.get("tools"):
                continue                              # 既にどうぐ有り
            key = (1 if op.in_play_area == AreaType.ACTIVE else 0,
                   1 if spot.get("id") == ARCH_EX else 0)
            if best_key is None or key > best_key:
                best_key, best = key, i
        return best

    def _pick_attach(self, idxs, options, hand, me):
        cape = self._pick_cape(idxs, options, hand, me)
        if cape is not None:
            return cape
