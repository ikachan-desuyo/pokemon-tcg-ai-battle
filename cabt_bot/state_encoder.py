...
# ランタイム必須データ。提出環境(Kaggle)ではCWD/配置が異なるため多パス探索し、
# 見つからなければ Fail Fast（silent fallback 禁止 — 「except→{}→line_threat=0」が
# v5〜v7の実戦で脅威ライン系機能を静かに無効化していた事故の再発防止）。
_CSV_CANDIDATES = [
    "input_data/extracted/JP_Card_Data.csv",                # ローカル(従来)
    "data/JP_Card_Data.csv",                                # 提出tar同梱(CWD基準)
    str(Path(__file__).resolve().parent.parent / "data" / "JP_Card_Data.csv"),  # パッケージ相対
    str(Path(__file__).resolve().parent.parent / "input_data" / "extracted" / "JP_Card_Data.csv"),
    "/kaggle_simulations/agent/data/JP_Card_Data.csv",      # Kaggle実行ディレクトリ
]
...
def _find_csv() -> str:
    for p in _CSV_CANDIDATES:
        if os.path.exists(p):
            return p
    raise RuntimeError(
        "JP_Card_Data.csv が見つかりません(ランタイム必須データ)。Fail Fast: "
        "silent fallbackで空データのままプレイすることを禁止しています。探索パス=" + repr(_CSV_CANDIDATES))


def _load_caps():
    rows = defaultdict(list)
    path = _find_csv()                                      # 無ければここで即失敗(意図的)
