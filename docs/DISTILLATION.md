...
| 土台優先展開・エネのアタッカー集中 | Lucario | ❌ | (play_priority/card_valuesの手書きが優位) | **Gap 23=最大の未蒸留知識** |
...
## 蒸留率スコアボード (2026-07-06, N=20につき参考値)
...
1. **Lucario(Gap23)**: 土台優先展開・エネ集中の蒸留。Universal最大の未習得知識
2. **Arch(被弾価値)**: 「自己スケール技持ちの温存」は新汎用ノブ候補=他デッキにも波及見込み
3. Dragapult(Gap4): 自然収束の可能性、様子見
4. Grimmsnarl(Gap9): 燃料ポケ還元で9まで縮小。次の抽象化を特定
5. Alakazam: 卒業手続き=導出ベース+差分構造への置換(N=50検収)
