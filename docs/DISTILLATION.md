...
### Residual Knowledge (一般化できなかった知識=将来のDeck Intent候補)

| 知識 | 発見元 | 状態 |
|---|---|---|
| Wildcard貼り分け(規則外エネ→主役)×手書き優先度の組合せ | Dragapult | 卒業保留デッキとして保持 |
| 被弾価値(傷んだ砲の温存) | Arch | 効果小(⑤56vs42, Gap寄与~1) |
| 開幕壁の条件(いつ壁が得か) | Lucario/Alakazam/Arch | archで逆効果の前科=条件未特定 |
| 自滅特性の認識+価値ゲート | 蒸留済 | Dragapult(「使うときぜつ」テキスト導出。従来は手書き専用) |
| コンボ素材の保持(アメ+Stage2→リーリエ抑制) | 蒸留済 | Dragapult(alakazam+4へ波及) |
| 二色コストの貼り分け(ワイルドカード主役規則) | **不採用** | Dragapult(単体移植で3デッキ微劣化=手書き要素との組合せ知識。Deck Intent残差候補) |
| 被弾価値(傷んだ砲の温存) | 未蒸留 | Arch(⑤ 56vs42、Gap寄与~1) |
| 開幕壁の選択 | 未蒸留(条件付き=archで逆効果の前科) | Lucario/Alakazam |

## 卒業証書 (2026-07-06, 各N=50検収)

| デッキ | 卒業形 | 検収 | 残差分(意味) |
|---|---|---|---|
| lucario | infer_plan+差分2行 | 85%(旧81) ✓ | 相方/土台cap=一般化候補 |
| alakazam | infer_plan+差分5行 | 77%(旧78) ✓ | 後攻/PH明示=固有、Telepath=データ欠如、壁=Residual候補、静的特性cap=一般化候補 |
| arch | infer_plan+差分1行 | 81%(旧83) ✓ | 静的特性(Memory Dive)cap=一般化候補 |
| grimmsnarl | infer_plan+差分1行 | 83%(旧86) ✓ | 静的特性/燃料ポケcap=一般化候補 |
| **dragapult** | **卒業保留(手書きPLAN維持)** | 卒業形78 vs 手書き87=劣化明確 | **Residual知識の保有デッキ**(Wildcard貼り分け×手書き優先度の組合せ) |

## Phase7総括

**「各デッキの勝ち方」を手書きPLANからUniversalへ蒸留し、Universalが知るべき知識と、
最後まで一般化できなかったResidual Knowledgeを切り分けた。
以後、新しい推論層はResidualが十分に蓄積した場合のみ追加する。**
