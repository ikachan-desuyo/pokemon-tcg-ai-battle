...
1. **卒業手続き**: 全デッキを `infer_plan(deck)+薄い差分` 形へ置換(Phase7完了条件)。
   測定分解能の知見: N=50でも総合値は±5前後揺れる(dragapult Uni 77-85帯を実測)→
   **Gap<5は0と区別不能=現行Inventoryで蒸留可能な知識は事実上尽きた**
2. 残差(二色貼り分け・被弾価値・開幕壁)はDeck Intent残差=「最後まで一般化できなかったもの」として次フェーズ(Universal Validation)の観測対象
