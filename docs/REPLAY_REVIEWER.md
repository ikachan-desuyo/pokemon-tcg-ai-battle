...
- ListEpisodes(submissionId) → kaggleusercontent リプレイ取得（既存 `research/ladder/ladder_meta.py`）。
...
|---|---|
| 0 | research/ladder/ladder_meta.py（取得・アーキ分類・勝敗） |
| 1a | research/ladder/ladder_dev_timing.py（WastedInvestment原型）/ research/ladder/ladder_three_findings.py（UnusedRight/MissedLethal原型） |
| 1b | evaluate_decision＋decision_diff（Kernel）/ research/episodes/episode4_behavior_diff.py（shadow比較） |
| 1c | research/ladder/ladder_loss_classify.py（敗因分類）/ research/ladder/ladder_meta.py（対面別） |
