...
## プロジェクト憲章
正式版は **[docs/CHARTER.md](CHARTER.md)**（目的 / 推論・開発・修正・採用・仮説の5原則 / 推論スタック）。
本書はその「修正の原則」「採用の原則」を実行するための手順書である。
（実践例: Comfeyでプラン崩壊を発見しても Episode 5 を止めず、Known Limitation として記録して
v7判定→Fetch Future Value の優先キューを維持した）
...
### Phase 6.5: 提出前QAゲート（必須・2026-07-03追加）
...
ローカル20〜50試合(ミラー+実ラダー復元ベンチ) → qa_gate.py(ReplayReviewer検出器)
  ...
  → 0件 → 提出可
```
- 人間がレビューで新種の違和感を見つけたら、まず**検出器として追加**し（受け入れ条件:
  その1試合で検出器も発火すること）、以後のQAゲートで自動検査する。
- 実績: 初回ゲート(30試合)で ValuelessSupport15件/LastStand11件/WallRetreat2件を検出=FAIL。
  これらは全て過去にKaggle提出後に人間が発見していた問題＝ローカルで見つけられたことの証明。
